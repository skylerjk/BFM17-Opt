...
FileLocs = {'../MS-Runs/BATS-29Prm-6objs-NrmRfSTD-Run6/PEOutput.dat', ...
    '../MS-Runs/BATS-29Prm-6objs-NrmRfSTD-Run7/PEOutput.dat'};
...
for i = 1:2
    ...
    CurFileLoc = FileLocs{i}
    
    temp = importdata(CurFileLoc,delimiter,hdr_lines);
    data = temp.data;
    ...
    figure(1);
    sc = plot([1:length(obj_val)],obj_val,'-o');
    ...
    figure(2)
    for j = 1:29;
        subplot(5,6,j), hold on
        sc = plot([1:length(obj_val)],prm_val(:,j),'-o');
...
% 
%     xlim([0 num_prm+0.5]), ylim([0.0 1.0])
% 
%     ax = gca;
%     ax.Color = FigClr; ax.XColor = AxsClr; ax.YColor = AxsClr;
