...
Check = NC_Tst_Data['P2l'][:]
if Check.shape[0] != 1080:
...
BGC_Ref_Fnl = np.zeros([5,depth,Num_Days])
BGC_Tst_Fnl = np.zeros([5,depth,Num_Days])

BGC_Ref_Avg = np.zeros([5,depth,12])
BGC_Tst_Avg = np.zeros([5,depth,12])
...
for fld in range(5):
    if fld < 4:
        BGC_Ref_Fnl[fld,:,:] = BGC_Ref_Data[fld,:,:]
        BGC_Tst_Fnl[fld,:,:] = BGC_Tst_Data[fld,:,:]
    else:
        BGC_Ref_Fnl[fld,:,:] = BGC_Ref_Data[4,:,:] + BGC_Ref_Data[5,:,:] + BGC_Ref_Data[6,:,:]
        BGC_Tst_Fnl[fld,:,:] = BGC_Tst_Data[4,:,:] + BGC_Tst_Data[5,:,:] + BGC_Tst_Data[6,:,:]

# Calculate Monthly Averages for data comparison - ref vs. test
for sv in range(5):
    for m in range(12):
        BGC_Ref_Avg[sv,:,m] = np.sum(BGC_Ref_Fnl[sv, :, 30*m:30*(m+1)], axis = 1)/30.
        BGC_Tst_Avg[sv,:,m] = np.sum(BGC_Tst_Fnl[sv, :, 30*m:30*(m+1)], axis = 1)/30.
