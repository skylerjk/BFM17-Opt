...
#endif
!#ifdef BFM_16
  use mem, ONLY: R6c,R1c,R2c,R6p,R1p,R6n,R1n,ppN1p,ppR1n,ppR1p,ppR6n,ppR6p, &
                 ppR6c,ppR1c
...
#endif
!#ifdef BFM_16
  real(RLEN), allocatable, dimension(:) :: fRIN1p,fRIN4n,fRIO3c
!#endif
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
