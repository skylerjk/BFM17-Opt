...
!#ifdef BFM_16
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  !NAMELIST PelChem_parameters,Pelchem_parameters_bfm16
  !-------------------------------------------------------------------------!
  ! Parameters for constant remineralisation of Organic matter
  real (RLEN) :: p_sR6O3
  real (RLEN) :: p_sR1O3
  real (RLEN) :: p_sR2O3
  real (RLEN) :: p_sR6N1
...
#endif
!#ifdef BFM_16
   namelist/PelChem_parameters_bfm_16/p_sR6O3,p_sR1O3,p_sR2O3,p_sR6N1,p_sR1N1, &
                                     p_sR6N4,p_sR1N4
!#endif
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
...
#ifdef BFM_16
  write(LOGUNIT,*) "#-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-"
  write(LOGUNIT,*) "#  Reading Constant Remineralisation parameters .."
  read(NMLUNIT,nml=PelChem_parameters_BFM_16,err=101)
#endif
  close(NMLUNIT)
...
#endif
#ifdef BFM_16
  write(LOGUNIT,nml=PelChem_parameters_BFM_16)
#endif
!
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  !END compute
